...
class JsonConfigResolver extends import_ConfigResolver.ConfigResolver {
  ...
  static addConfigs(priority, jsonObject) {
    ...
    for (const [configName, cfg] of Object.entries(
      ...
      try {
        ...
        const isStateless = JsonConfigResolver._parseStatelessString(
          /** @type {Record<string, unknown>} */
          cfg
        );
        const config = new import_Config.Config(runtimeName, connectionData, isStateless, plugins, modules);
   ...
   * @private
   */
  static _parseStatelessString(obj) {
    const v = JsonConfigResolver._getOptionalString(obj, "stateless");
